...
def get(seed=0, fixed_order=False, pc_valid=0.15, base_dir=None):
    ...
    idata=np.arange(8)#生成[0,1,2,3,4,5,6,7]
    ...
    if not os.path.isdir(base_dir+'/data/binary_mixture/'):
        ...
        for n,idx in enumerate(idata):
            if idx==0:
                ...
                data[n]={}#data{0:{name:XXX;ncla:XXX},1:{},2{}...}
                data[n]['name']='cifar10'
                data[n]['ncla']=10
                for s in ['train','test']:
                    # loader=torch.utils.data.DataLoader(dat[s],batch_size=1,shuffle=False)
                    data[n][s]={'x': [],'y': []}
                    for image,target in dat[s]:
                        data[n][s]['x'].append(np.asarray(image))
                        data[n][s]['y'].append(target)
                ...
                dat['train']=datasets.MNIST(base_dir+'/data/',train=True,download=True,transform=transforms.Compose([
                    transforms.Pad(padding=2,fill=0)])) #,transforms.Normalize(mean,std),transforms.ToTensor()
                ...
                for s in ['train','test']:
                    ...
                    data[n][s]={'x': [],'y': []}
                    for image,target in dat[s]:
                        image = np.reshape(np.stack([image,image,image], axis=-1), (32,32,3))
                ...
                dat={}
                dat['train']=TrafficSigns(base_dir+'/data/traffic_signs', train=True, download=True) #,transforms.Normalize(mean,std), transform=transforms.Compose([transforms.ToTensor()])
                dat['test']=TrafficSigns(base_dir+'/data/traffic_signs', train=False, download=True) #,transforms.Normalize(mean,std), transform=transforms.Compose([transforms.ToTensor()])
                ...
                dat={}
                dat['train']=Facescrub(base_dir+'/data/facescrub', train=True, download=True)#,transforms.Normalize(mean,std), transform=transforms.Compose([transforms.ToTensor()])
                dat['test']=Facescrub(base_dir+'/data/facescrub', train=False, download=True)#,transforms.Normalize(mean,std), transform=transforms.Compose([transforms.ToTensor()])
                data[n]={}
                ...
                std=(0.4501,)
                dat={}
                dat['train']=notMNIST(base_dir+'/data/notmnist', train=True, download=True) #,transforms.Normalize(mean,std), transform=transforms.Compose([transforms.ToTensor()])
                dat['test']=notMNIST(base_dir+'/data/notmnist', train=False, download=True) #,transforms.Normalize(mean,std), transform=transforms.Compose([transforms.ToTensor()])
                data[n]={}
                ...
                for s in ['train','test']:
                    ...
                    data[n][s] = {'x': [], 'y': []}
                    for image,target in dat[s]:
                        image = np.reshape(np.concatenate((image,)*3, axis=-1), (32,32,3))
                        data[n][s]['x'].append(np.asarray(image))
                        data[n][s]['y'].append(target)
                    # for image, target in loader:
            ...
            for s in ['train','test']:
                ...
                # pdb.set_trace()
                # data[n][s]['x'] = np.reshape(data[n][s]['x'], (-1,size[0],size[1],size[2]))
                # data[n][s]['y'] = np.array(data[n][s]['y'], dtype=int)
